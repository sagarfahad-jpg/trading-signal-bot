...
## 1️⃣1️⃣ المرحلة ٢ — وضع الظل لبوابة الدخول بالتأكيد عند المنطقة (منذ 2026-09-12)

**لماذا:** الحكم على 636 صفقة مقاسة بالعقد (06-30 → 09-11): win-rate 24% مقابل تعادل 30%، −5.2%/صفقة. التشخيص: البوت لا ينفّذ استراتيجية المالك — التأكيد يُحسب لحظة المسح كمكافأة سكور (97% من الإشارات `cisd=False`)، واللمسة الأولى لذيل شمعة 1m على شريط ضحل (0.2×ATR تحت السعر) = دخول بلا شرط، فتُغلق 69% من الصفقات خلال 30 دقيقة.

**ما بُني (قارئ بحت، لا يلمس الدخول الحي):**
- `entry_gate.py` — المنطق النقي: على شموع 5m **مغلقة** داخل الجلسة، `armed` → أول شمعة تتداخل مع حدود منطقة HTF الفعلية = `touched` (شمعة ١ من ٦) → تأكيد بالترتيب `rejection` (جديد: ذيل يكسر حدّ المنطقة، أطول من الجسم ومن أطول ذيل مماثل في آخر 5 شموع، إغلاق داخل/خلف المنطقة) → `cisd` → `ifvg` → `fvg` (الثلاثة واعية بشمعة الوصول) → `entered` بسعر = إغلاق الشمعة المؤكِّدة. الإلغاء: إغلاق خلف المنطقة (`break`)، 6 شموع بلا تأكيد (`timeout`)، أو إغلاق بعد 15:00 ET (`eod`، بلا حمل لليوم التالي). `displacement` علم مسجَّل لا يُدخِل.
- الستوب: هامش 0.15×ATR(14, 5m). `rejection` → خلف الشمعة الرافضة؛ `cisd/fvg/ifvg` → خلف حدّ المنطقة؛ منطقة عرضها > 1×ATR → خلف الشمعة المؤكِّدة أياً كان النوع (المسار الشائع: 82% من المناطق الموافقة يومية).
- الأهداف هيكلية: أقرب مستوى يبعد ≥ max(1R, 0.5×ATR) وضمن 4R من {PDH/PDL، PWH/PWL، PMH/PML، swing 5m/1h، strong/weak/trailing، EQH/EQL، حافة أقرب منطقة HTF معاكسة}؛ لا مستوى → 2R موسوم `fallback_2r` (في الظل فقط؛ عند التحويل الحي: لا دخول).
- `shadow_gate.py` — الخيط: كل حدّ 5 دقائق + 20 ثانية جلب مجمّع للشموع وتقييم الجديد منها بكتابة مشروطة بالحالة (ذرّي)، وصفقات الظل النشطة كل 45 ثانية بنفس قواعد `price_monitor._check` مع سعر العقد الحقيقي. الحقيقة كلها في الصف (`last_bar_ts`, `bars_seen`, `arrival_bar_ts`…) فتنجو إعادة تشغيل Railway بلا تكرار ولا ضياع؛ فجوة > 3 دقائق → حكم تحفّظي من الشموع الفائتة (الستوب أولاً، `gap=true`)؛ تأكيد أقدم من 15 دقيقة → `stale`.
- التسجيل: `main.scan()` بعد حفظ الإشارة → `shadow_gate.register` يصنّف: `no_zone` (2% تاريخياً) · `zone_conflict` (42%، الأسوأ أداءً −11.5%/صفقة؛ يُسجَّل هل وُجدت منطقة موافقة بديلة أضعف — تحليل فقط، بلا إنقاذ) · `armed` (55%).
- التخزين: `shadow_watches` + `shadow_events` في Supabase (`migrations/001_shadow_gate.sql`). مفتاح إيقاف `bot_config.shadow_gate`. الرسالة الوحيدة: ملخّص يومي 15:52 ET.
- المقارنة بعد أسبوعين: `analysis/shadow_compare.sql` (join على `signal_id`؛ استعلام منفصل لخسائر `zone_conflict` التي تجنّبها الظل بمجرد عدم المراقبة، وآخر لأثر التأكيد على الفئة الموافقة). ثم قرار التحويل الحي.

**مؤجَّل صراحة:** الزيارة الثانية (يُقاس فقط كم `timeout` يتبعه رجوع للمنطقة)، شرط الفوليوم، ترجيح التأكيدات، تعديلات فلتر الوقت، إعادة معايرة الرموز.

---

## ⚠️ ألغام قبل التحويل الحي (يجب إصلاحها قبل أن تصير البوابة هي الدخول الفعلي)

1. **`_get_perf_adj` يعدّ `cancelled` خسائر** (`analyzer.py:398`): يتخطّى `open/expired` فقط، فكل صف `cancelled` (إلغاء 24 ساعة اليوم، و`break/timeout/eod` غداً) يُحسب صفقة محسومة بلا فوز ويخفض سكور نوع الدخول تلقائياً حتى −1.0. مع بوابة تلغي معظم الزيارات سيقتل السكور. الإصلاح: استبعاد `cancelled`/`cancel_requested`/`exit_requested` من العدّ (ومعاملة `manual_exit` بحسب `r_multiple`).
2. **`price_in_zone` يعيد أقوى منطقة تحتوي السعر بغض النظر عن اتجاهها** (`htf_zones.py:129`): مناطق daily INV_FVG بقوة 3.5 تفوز دائماً، فيخرج 42% من الإشارات بمنطقة معاكسة (بوت داخل demand: 254، كول داخل supply: 185) بعقوبة −2 فقط، وهي الفئة الأسوأ أداءً. الإصلاح: اختيار المنطقة بعد الاتجاه النهائي ومن المناطق الموافقة فقط (أو رفض الإشارة عند التعارض)، وقياس أثره على الظل أولاً عبر `alt_zone_found`.
3. عند التحويل: إضافة `watching` لقائمة `has_open_signal` (`db.py:166`) وإلا تتكرر إشارات الرمز كل مسح؛ تسمية الحالة في الداشبورد؛ عدّها في الملخّص اليومي؛ فتح `expire_stale` على الحالات الجديدة.
